...
# Force Hugging Face caches to the shared dataset directory if present so that
# model and dataset downloads reuse the bundled cache at /workspace/datasets/hf_cache.
_HF_CACHE_ROOT = os.getenv("HF_HOME", "/workspace/datasets/hf_cache")
if _HF_CACHE_ROOT and pathlib.Path(_HF_CACHE_ROOT).exists():
    os.environ.setdefault("HF_HOME", _HF_CACHE_ROOT)
    os.environ.setdefault("HUGGINGFACE_HUB_CACHE", str(pathlib.Path(_HF_CACHE_ROOT) / "hub"))
    os.environ.setdefault("TRANSFORMERS_CACHE", str(pathlib.Path(_HF_CACHE_ROOT) / "hub"))
    os.environ.setdefault("HF_DATASETS_CACHE", str(pathlib.Path(_HF_CACHE_ROOT) / "datasets"))
