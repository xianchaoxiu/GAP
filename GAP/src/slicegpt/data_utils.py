...
def _load_dataset_local_first(path: str, config_name: str | None = None, data_files=None):
    """Try to load dataset from local HF Hub cache before hitting the network."""
    hf_home = _os.environ.get("HF_HOME", "/workspace/datasets/hf_cache")
    hub_dir = _pathlib.Path(hf_home) / "hub"
    # Old arrow-format cache (datasets < 3.x)
    datasets_cache = _pathlib.Path(hf_home) / "datasets"

    # Build dataset key for hub cache (e.g. datasets--wikitext)
    hub_key = "datasets--" + path.replace("/", "--")
    snapshots_dir = hub_dir / hub_key / "snapshots"

    if snapshots_dir.exists():
        # Find snapshot with the needed config split files
        for snap in sorted(snapshots_dir.iterdir(), reverse=True):
            data_dir = snap / (config_name or "")
            if not data_dir.exists():
                data_dir = snap
            parquets = list(data_dir.glob("*.parquet"))
            if parquets:
                # Build data_files dict from parquets
                split_files: dict[str, list[str]] = {}
                for p in parquets:
                    name = p.stem  # e.g. 'train-00000-of-00001'
                    split = name.split("-")[0]
                    split_files.setdefault(split, []).append(str(p))
                logging.info(f"Loading {path}/{config_name} from local parquet cache: {data_dir}")
                ds = datasets.load_dataset("parquet", data_files=split_files)
                # Rename columns to match wikitext2 format: 'text' column
                return ds

    # Fall back to standard load_dataset (may be slow on no-internet server)
    if data_files is not None:
        return datasets.load_dataset(path, name=config_name, data_files=data_files)
    return datasets.load_dataset(path, name=config_name)
...
def get_dataset(name: str) -> datasets.DatasetDict:
    ...
    if properties.get("data_files") is None:
        ds = _load_dataset_local_first(
            properties["path"],
            config_name=properties.get("config_name"),
        )
    else:
